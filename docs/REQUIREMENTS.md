...
| AC-26 | The gemini brand card renders the same two-bar template (5-Hour and Weekly, with `%` fill and reset badge) as the claude / minimax / glm cards. The `isAntiqravity` branch in `renderBrandCards()` is gone | Manual: load dashboard; code review |
| AC-27 | `lib/antigravity-parser.js countTokensFor(text)` returns the exact Gemini token count when `GEMINI_API_KEY` is set (and the SDK loads cleanly), and falls back to `Math.ceil(text.length / 4)` when no key is set or the API call errors. Cache hits on identical text return the cached value | Vitest: `tests/antigravityParser.test.js` covers heuristic, injected client, cache, and error-fallback cases |
| AC-28 | `GET /api/agent-usage` returns a `contextWindow` object with `{used, remaining, usedPct, size, source, lastUpdated}` only when an `agent_usage` row has been updated within the last `ACTIVE_SESSION_MS = 30 minutes`. When no recent row exists, `contextWindow` is `null`. `size` defaults to `1_000_000` and is overridable via the `GEMINI_CONTEXT_WINDOW` env var | Vitest: `tests/antigravityContext.test.js` covers empty-rows, sqlite-error, numerator excludes outputTokens, 100% clamp, explicit-size override |
