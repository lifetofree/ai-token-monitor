...
- [x] **💻 TDD Engineer**: Implemented formatters, simulation engine, settings form, **Real RTK mode with SSE streaming**, **provider-quota tracking with live MiniMax fetcher**, and **API-driven progress bar**. **Vitest suite scaffold landed**: `package.json` adds `vitest ^1.6.0` devDep and `check`/`test`/`test:watch` scripts; `vitest.config.js` is in place; `tests/format.test.js`, `tests/cost.test.js`, `tests/detectBrand.test.js`, `tests/computeApiUsedPct.test.js`, `tests/fetchMinimaxQuota.test.js`, and `tests/csv.test.js` are all created. The mirror-function approach is documented at the top of each test file; a follow-up task is to extract the canonical implementations into a shared `lib/` module that both `app.js` and the tests can import.
