...
function getClaudeRealRateLimits() {
  ...
  // When multiple Claude Code sessions are active simultaneously, each session
  // receives independently-timed rate_limits updates from Anthropic's server.
  // Picking the "freshest" file causes the card to flicker between wildly
  // different values (e.g. 30% ↔ 66% ↔ 93%) as different sessions write
  // their cache files at different times.
  //
  // The real account-wide 5h limit is a single shared pool. The session that
  // reports the HIGHEST used_percentage has the most up-to-date view of how
  // much has actually been consumed (older/staler sessions undercount because
  // they haven't received recent server updates yet). Always pick the maximum
  // to show a safe, conservative, non-flickering value.
  let best = null;
  for (const dirName of dirs) {
    const candidate = readSessionFile(dirName);
    if (!candidate) continue;
    const candidateUsed = candidate.data.rate_limits.five_hour.used_percentage;
    const bestUsed = best ? best.data.rate_limits.five_hour.used_percentage : -1;
    if (candidateUsed > bestUsed) best = candidate;
  }
  if (!best) return null;
