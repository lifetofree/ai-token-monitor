...
(function (root, factory) {
  ...
  // drive the bar for this scope (the renderer then falls back to RTK or
  // local rolling spend). unit values that yield null:
  //   - 'local'      (Claude): tracked purely via RTK, no provider window.
  //   - 'per_minute' was removed when Claude became RTK-only.
  ...
  function computeApiUsedPct(apiQuota, scope) {
    ...
    if (scope === '5h') {
      ...
      // 'local' (Claude) is tracked purely via RTK; no provider 5h window.
