...
async function publishToFirebase(results, env, allSpend) {
  ...
  for (const r of results) {
    ...
    // Synthesize remaining/limit_value/weekly_remaining for brands whose
    // provider doesn't expose quota (unit === 'not_exposed'). The local-spend
    // percentages below are computed from real cost vs configured limit, so
    // we derive "X% remaining" from them — keeps the ESP32 firmware rendering
    // meaningful numbers instead of "-1" sentinels.
    const effectiveUnit = r.unit || 'not_exposed';
    const localPct5h     = Math.round(calcSpendPct(c5, lim.limit5h));
    const localPctWeekly = Math.round(calcSpendPct(cW, lim.limitWeekly));
    const rem5h          = effectiveUnit === 'not_exposed' ? Math.max(0, 100 - localPct5h)     : (r.remaining        !== null ? r.remaining        : -1);
    const remWk          = effectiveUnit === 'not_exposed' ? Math.max(0, 100 - localPctWeekly) : (r.weekly_remaining !== null ? r.weekly_remaining : -1);
    const limValue       = effectiveUnit === 'not_exposed' ? 100                              : (r.limit_value      !== null ? r.limit_value      : -1);

    payload.quotas[r.brand] = {
      name:             NAMES[r.brand] || r.brand,
      remaining:        rem5h,
      limit_value:      limValue,
      weekly_remaining: remWk,
      unit:             effectiveUnit,
...
// Publishes a Mac metrics sample to /display/mac — a sibling of /display/quotas,
// not nested inside it (wayfinder issue #6), so this is a direct PUT with no
// read-merge-write step against the quota snapshot.
async function publishMacToFirebase(macSnapshot, env) {
  const dbUrl  = env.FIREBASE_URL  || env.FIREBASE_DB_URL  || process.env.FIREBASE_URL  || process.env.FIREBASE_DB_URL;
  const secret = env.FIREBASE_AUTH || env.FIREBASE_DB_SECRET || process.env.FIREBASE_AUTH || process.env.FIREBASE_DB_SECRET;
  if (!dbUrl || !secret) return;

  const firebaseUrl = `${dbUrl.replace(/\/$/, '')}/display/mac.json?auth=${encodeURIComponent(secret)}`;
  const res = await fetch(firebaseUrl, {
    method:  'PUT',
    headers: { 'Content-Type': 'application/json' },
    body:    JSON.stringify(macSnapshot),
    signal:  AbortSignal.timeout(8000),
  });
  if (!res.ok) console.error(`[firebase] PUT mac ${res.status}`);
}

module.exports = { publishToFirebase, publishMacToFirebase };
