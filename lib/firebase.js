...
async function publishToFirebase(results, env, allSpend) {
  ...
  const NAMES = { gemini: 'Antigravity', claude: 'Claude', minimax: 'Minimax', glm: 'GLM', mimo: 'MiMo' };
  const SPEND_LIMITS = {
    ...
    mimo:    { limit5h: 2.00,  limitWeekly: 15.00 },
  };
  ...
  for (const r of results) {
    ...
    // Extract _rtk_spend when present (used by GLM, MiMo, and other
    // brands that embed RTK spend data in raw_json).
    if (rtk && rtk._rtk_spend) {
      rtk = rtk._rtk_spend;
    }
