...
function fetchClaudeQuota(apiKey, rtkSpend) {
  // Claude is tracked purely from the local RTK database. The Anthropic API
  // does not expose a usable quota window: the rate-limit headers are a
  // per-minute token bucket (not 5h/weekly) and are absent when the account
  // has insufficient credit (the probe 400s before any header is set).
  // RTK already records every Claude Code call (input/output/saved tokens,
  // timestamps), so the dashboard's rolling-window spend, request count, and
  // reset time are all derived locally — no outbound probe required.
  //
