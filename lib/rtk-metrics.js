  ...
  return new Promise((resolve) => {
    ...
    execFile('sqlite3', ['-cmd', '.timeout 5000', '-json', DB_PATH, query], (error, stdout) => {
      ...
      try {
        ...
        // Brand detection for RTK spend metrics. Falls back to 'claude' because
        // RTK only records commands proxied through Claude Code — unmatched commands
        // (grep, ls, cat, tsc, etc.) are Claude Code tool calls without an explicit
        // 'anthropic' marker. The 0-token guard below filters out pure-noise rows
        // that have no billing impact. See also detectBrand() in app.js which
