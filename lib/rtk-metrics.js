  ...
  return new Promise((resolve) => {
    ...
    execFile('sqlite3', ['-cmd', '.timeout 5000', '-json', DB_PATH, query], (error, stdout) => {
      ...
      try {
        ...
        // Brand detection for RTK spend metrics. Returns null for shell commands
        // (grep, ls, curl, git, npm, etc.) so they are excluded from cost
        // aggregation. Only commands with explicit brand markers (anthropic, gemini,
        // minimax, glm) are counted. See also detectBrand() in app.js.
        ...
        rows.forEach(row => {
          ...
          if (!brandKey) return; // skip unmatched (shell commands, tool calls)
